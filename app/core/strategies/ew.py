...
def HRP(strategy: Strategy) -> Optional[pd.Series]:
    ...
    if reb_prices is None:
        return None
    return Optimizer.from_prices(prices=reb_prices).hierarchical_risk_parity()
...
def RP(strategy: Strategy) -> Optional[pd.Series]:
    ...
    if reb_prices is None:
        return None
    return Optimizer.from_prices(prices=reb_prices).risk_parity()
...
def MaxSharpe(strategy: Strategy) -> Optional[pd.Series]:
    ...
    if reb_prices is None:
        return None
    return Optimizer.from_prices(prices=reb_prices).maximized_sharpe_ratio()
...
def InvVariance(strategy: Strategy) -> Optional[pd.Series]:
    ...
    if reb_prices is None:
        return None
    return Optimizer.from_prices(prices=reb_prices).inverse_variance()
