...
def get_prices(tickers: Union[str, List, Set, Tuple]) -> pd.DataFrame:
    ...
    out = pd.concat([
        get_price(ticker) for ticker in tickers
    ], axis=1)
    return out
...
def fred_data(ticker: str) -> pd.Series:
    """get the fred data"""
    out = pdr.DataReader(
        name=ticker, data_source="fred", start=datetime(1900, 1, 1),
    )
...
def get_macro(tickers: Union[str, List, Set, Tuple]) -> pd.DataFrame:
    if isinstance(tickers, dict):
        out = get_macro(tickers=list(tickers.keys()))
        out = out.rename(columns=tickers)
        return out
    # create ticker list
    tickers = (
    ...
    return pd.concat([
        fred_data(ticker) for ticker in tickers
    ], axis=1)
